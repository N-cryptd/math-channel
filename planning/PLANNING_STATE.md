...
- NEXT: SP cohort 237-240 pacing audits — measure naturals with DIRECT single-clip edge-tts (en-US-AndrewNeural --rate=-5%) vs shipped SRT spans; FOUR gates: (a) span < natural+0.3 (defect); (b) ratio natural/span ≤ 0.93 (healthy); (c) declared ≥ natural per caption (else narrate speedup — check media/narration_speedup_warnings.log on every render; >1.08x logs); (d) usage = natural/min(declared_end, next_start−0.3, video_dur) ≤ 0.97 per caption (catches overlapping declared ends and silent sub-1.08x speedups); PLUS the triage shortcut both ways (sum(naturals) vs shipped timeline: over ⇒ expect every slot defective [235], under ⇒ expect localized defects [236]); fix recipe + caption→gap attribution rules in improvement-tracker.md SP audit section (2026-09-10). If shipped SRT is missing from media/ (cleanup), re-render baseline FIRST to regenerate it (231 protocol).
