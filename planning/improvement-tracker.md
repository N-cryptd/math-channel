...
| 83 | Equivalence Relations | 770 | YES | YES (content budget) | N/A | v2-compliant. Fixed 6 content budget violations (scenes 1,3,4,5,6,8) by removing bridge in scene1, adding FadeOut before verdict in scene3, FadeOut(desc,example_set,boxed_def) before class2 in scene4, FadeOut(boxed_cong,prop_check) before class1 in scene5, FadeOut(reqs) before formal in scene6, FadeOut(parts1-2) before part3 in scene8. Added formula_box for [a] definition and mod n congruence. setup_bg(2), section_divider(8), SANS(39), ly.clear(12), progressive_reveal(1), safe_place(52), formula_box(2), zero .shift()/.to_edge()/.move_to()/.next_to(). |
