...
// Usage: node search_topic.js "search query" [--limit N]
...
async function searchTopic(query, limit) {
    ...
    const items = resp?.data?.contents?.twoColumnSearchResultsRenderer
    ...
    for (const item of items) {
        ...
        results.push({
            ...
            views: vid.viewCountText?.simpleText || 
                   vid.viewCountText?.runs?.map(r=>r.text).join('') || '',
...
async function main() {
    ...
    process.stderr.write(`Searching: "${query}" (limit ${limit})...\n`);
    const results = await searchTopic(query, limit);
